...
- **Never run `gh` inside the macOS sandbox. Every GitHub CLI invocation must use elevated sandbox permissions.** This machine's valid `gh` credential is stored in the macOS Keychain, which sandboxed commands cannot reliably access. A sandboxed `gh auth status` may falsely report an invalid or expired token, and other sandboxed `gh` commands can fail for the same reason. Never diagnose or report a GitHub credential problem from a sandboxed result; discard that result and rerun the command outside the sandbox. As of 2026-09-04, an unsandboxed `gh auth status` succeeds for the active `bendymike` account with `repo`, `workflow`, `read:org`, and `gist` scopes.
...
- **Dependency mutation requires explicit user authorization.** A pnpm "out of sync," "workspace structure changed," or stale-dependency warning is informational. It never authorizes an install, repair, relink, clean, rebuild, or any other `node_modules` mutation. Do not run `pnpm deps:install`, `pnpm deps:repair`, `pnpm install`, `pnpm rebuild`, or a command that may install dependencies unless the user explicitly requested dependency work. For lint, formatting, tests, and diagnosis, use existing local binaries when available. If a required binary is genuinely missing, report that verification is blocked and ask before installing. Never repair dependencies merely to remove a warning.
- **Never run a bare `pnpm install` in this shared checkout.** Multiple Codex tasks can run here at once, while pnpm rewrites the same live `node_modules` tree non-atomically. The root `pnpm:devPreinstall` guard rejects accidental bare local installs before mutation begins. `verifyDepsBeforeRun: warn` is deliberate: build/test/exec commands may report stale metadata, but must never silently turn into installers.
- `pnpm deps:status` — read-only dependency diagnosis. Run this when a pnpm warning needs explanation.
- `pnpm deps:install` — explicit missing-dependency bootstrap. It is a no-op when required workspace links and binaries already exist. It fetches and verifies the frozen lockfile before touching `node_modules`.
- `pnpm deps:repair` — explicit destructive reconciliation. It requires interactive confirmation (or `--confirm-repair` after explicit user authorization), waits for shared build/test readers to finish, fetches before mutation, and only then permits pnpm to rebuild `node_modules`. If an authorized dependency operation is started, allow at least 30 minutes and do not interrupt it after mutation begins.
- Build, test, lint, typecheck, dev, and packaging commands hold shared dependency-read leases. Dependency installs, repairs, Gilde/Squisq link changes, and `gilde:update` hold the exclusive mutation lease. Separate worktrees should use separate `node_modules`; pnpm's content-addressable store remains shared across them.
