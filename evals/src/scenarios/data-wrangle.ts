...
export const DATA_WRANGLE_KICKOFF_MESSAGE = [
  ...
  'writeFile are not available inside Node scripts. Do not paste line-numbered readFile output',
...
function dataWrangleRepairDirectiveBase(result: CustomersCheckResult): string {
  ...
  if (missing.has('parses-as-array')) {
    return [
      ...
      'Inside a Node script, read the raw CSV files from disk with fs.readFileSync. Do not paste line-numbered readFile output into string constants, and do not call MCP tools like writeFile from inside the script.',
  ...
  if (missing.has('source-ids-preserved')) {
    return [
      ...
      'If using a Node helper, use fs.readFileSync to read data/raw/*.csv from disk and fs.writeFileSync to write out/customers.json. MCP tools like writeFile are not functions inside Node scripts.',
      'Do not paste line-numbered readFile output into script string constants; those line markers are not part of the CSV data.',
  ...
  if ([...missing].some((signal) => structuralSignals.has(signal))) {
    return [
      ...
      'If using a Node helper, use fs.readFileSync/fs.writeFileSync. Do not paste line-numbered readFile output into a JS string, and do not call MCP tools like writeFile from inside Node.',
