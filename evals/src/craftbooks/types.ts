...
export interface CraftbookEvalFixtureFile {
  ...
  modelInput?: boolean;
  /**
   * Base64 bytes to seed instead of `content`, for source documents that must
   * be a real container — a PDF or DOCX the craftbook has to open with
   * `read_doc_as_markdown` rather than `read_file`. `content` stays required
   * and carries the plain-text equivalent: graders and the citation checker
   * read it, and it documents in the spec what the bytes actually say.
   *
   * Eval-side only. The published `CraftbookTestSpec` takes text fixtures
   * because a `test.json` is hand-authored data; these bytes are generated by
   * the scenario at build time, which is the whole reason they live here.
   */
  contentBase64?: string;
  /** Content type for the raw write; defaults to application/octet-stream. */
  mimeType?: string;
}
