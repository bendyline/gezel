...
async function main(): Promise<void> {
  ...
  console.log(`Craftbook local-model batch plan (${plan.items.length}/${plan.target})`);
  ...
  for (const item of plan.items) {
    ...
    console.log(
      `  ${String(item.priority).padStart(3)} ${item.craftbookId.padEnd(28)} ${item.harness.join('+')}${simulators}`,
    );
