...
public class MobileModelStore {
    public static final int MAX_STATE = 16 * 1024 * 1024;
    public static final int MAX_LIBRARY = 1024 * 1024;
    public static final int MAX_MODELS = 100;
    public static final long MAX_MODEL = 4L * 1024 * 1024 * 1024;
    ...
    protected static String decode(byte[] bytes) throws IOException {
        return StandardCharsets.UTF_8.newDecoder().onMalformedInput(CodingErrorAction.REPORT)
            .onUnmappableCharacter(CodingErrorAction.REPORT).decode(ByteBuffer.wrap(bytes)).toString();
    }

    protected byte[] readDocument(String name, int maximum) throws IOException {
    ...
    protected void atomicWrite(File destination, byte[] bytes) throws IOException {
    ...
    public static JSONObject validateLibrary(JSONObject value) throws Exception {
    ...
    public synchronized File downloadsRoot() throws IOException {
    ...
    public synchronized File downloadModelPath(String id) throws Exception { return modelPath(id); }
    /** Called only after the transfer manager independently hashes this private file. */
    public synchronized NativeObject publishDownloadedModel(String id, String name, JSONObject source, File partial) throws Exception {
        ...
        for (int i=0; i<entries.length(); i++) if (entries.getJSONObject(i).getString("id").equals(id)) {
            ...
            return NativeObject.fromJSONObject(existing);
        }
        ...
        return NativeObject.fromJSONObject(model);
    }

    public synchronized NativeObject listModels() throws Exception { return NativeObject.fromJSONObject(library()); }

    public synchronized NativeObject selectModel(String id) throws Exception {
    ...
    public synchronized void removeModel(String id) throws Exception {
        JSONObject library = library();
    ...
    public synchronized String[] model(String id) throws Exception {
        JSONObject entry = findModel(library(), id);
        return new String[] { entry.getString("id"), checkedPath(entry).getAbsolutePath() };
    }

    public synchronized String[] selectedModel() throws Exception {
        JSONObject library = library();
    ...
    public synchronized NativeObject importModel(ContentResolver resolver, Uri uri) throws Exception {
        JSONObject library = library();
