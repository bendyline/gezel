...
// Also used by the standalone SDK packager before copying any native bytes.
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  const [target, build] = process.argv.slice(2);
  if (!['android', 'ios'].includes(target) || !build)
    throw new Error('Usage: node native/mobile/verify-build.mjs android|ios <build-directory>');
  const repo = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../..');
  const manifest = await verifyNativeBuild(repo, path.resolve(build), target);
