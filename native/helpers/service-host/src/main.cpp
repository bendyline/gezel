...
std::vector<EnvEntry> env_overrides(const std::wstring& instdir, const std::wstring& home) {
  ...
  return {
      ...
      {L"GEZEL_PNPM_PATH", unpacked + L"\\dist\\pnpm-bundle\\pnpm.exe"},
...
void print_help() {
  std::cout << "Gezel service host\n\n"
            ...
            << "Windows Service Control Manager. This binary launches nothing but\n"
            << "Gezel's own daemon — it has no general-purpose mode.\n\n"
            << "Usage:\n"
...
int self_test() {
  ...
  const std::vector<std::pair<std::wstring, std::wstring>> expected_env = {
      ...
      {L"GEZEL_PNPM_PATH",
       L"C:\\Program Files\\Gezel\\resources\\app.asar.unpacked\\dist\\pnpm-bundle\\pnpm.exe"},
      {L"GEZEL_NODE_PATH",
