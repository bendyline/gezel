...
application runtimes** above rather than here: like Node.js and pnpm it is a
...
The iOS and Android apps compile their own native stack rather than reusing the
desktop server binaries. Upstream pins live in
[`native/mobile/speech/pins.json`](native/mobile/speech/pins.json) and, for the
inference engine, the same [`native/engines/llama-cpp/VERSION`](native/engines/llama-cpp/VERSION)
the desktop build reads. Every license text below is staged into the app bundle
under `licenses/native/` and is checked at packaging time by
`packages/mobile/scripts/verify-android-package.py` and `verify-ios-archive.py`.

| Component | Pinned version | License | Source |
|---|---|---|---|
| **llama.cpp** (`libgezel-llama`, compiled into the app) | same pin as the desktop engine | MIT | [ggml-org/llama.cpp](https://github.com/ggml-org/llama.cpp) |
| **ggml** | bundled with llama.cpp/whisper.cpp | MIT | [ggml-org/ggml](https://github.com/ggml-org/ggml) |
| **whisper.cpp** (`libGezelSpeech`, speech-to-text) | commit `f049fff9` | MIT | [ggml-org/whisper.cpp](https://github.com/ggml-org/whisper.cpp) |
| **ONNX Runtime** (`libonnxruntime.so`, `onnxruntime.xcframework`) | `1.28.1` | MIT | [microsoft/onnxruntime](https://github.com/microsoft/onnxruntime), built by [csukuangfj/onnxruntime-libs](https://github.com/csukuangfj/onnxruntime-libs) |
| **Capacitor** (app shell, Android and iOS runtimes) | `8.5.2` | MIT | [ionic-team/capacitor](https://github.com/ionic-team/capacitor) |

Apple Foundation Models and Android ML Kit are operating-system APIs the app
calls. Nothing from either is redistributed.

### No eSpeak NG in the speech pipeline

Kokoro text-to-speech runs on ONNX Runtime directly. It previously ran through
sherpa-onnx, whose text-to-speech build links **eSpeak NG (GPL-3)** whether or
not anything calls it, so the engine would have shipped inside an MIT app bound
for the App Store and Google Play. sherpa-onnx served only Kokoro here —
recognition already calls whisper.cpp directly — so it was removed, together
with the `espeak-ng-data` directory the voice pack carries and the
piper-phonemize grapheme-to-phoneme layer. None of the three is distributed in
any build.

The desktop daemon had the same exposure by a different route: `kokoro-js`
depends on `phonemizer`, which embeds eSpeak NG compiled to WebAssembly behind
an Apache-2.0 package name. The private workspace package
[`packages/phonemizer-compat`](packages/phonemizer-compat/README.md) occupies
that dependency slot, so the engine is never installed or shipped, and any call
into it throws. This mirrors the existing `packages/sharp-compat` stub.

Both hosts now turn text into phonemes with `@bendyline/gezel/kokoro`
([`packages/core/src/kokoro`](packages/core/src/kokoro)), first-party code
under Gezel's own license.

### Bundled speech models and data

Unlike the desktop models below, these ship inside the mobile app bundle.

| Asset | Used for | License | Source |
|---|---|---|---|
| **Whisper `tiny`** (multilingual GGML) | on-device speech-to-text | MIT | [ggerganov/whisper.cpp](https://huggingface.co/ggerganov/whisper.cpp) |
| **Kokoro-82M int8 multi-lang v1.0** voice pack (model, voices, tokens) | on-device text-to-speech | Apache-2.0 | [k2-fsa/sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx/releases/tag/tts-models) release asset |
| **Kokoro pronunciation dictionaries** (`lexicon-us-en`, `lexicon-gb-en`, `lexicon-zh`) | turning text into phonemes | Apache-2.0 | same voice pack; English entries derive from [hexgrad/misaki](https://github.com/hexgrad/misaki) |
...
eSpeak NG over a word list ([hexgrad/misaki#51](https://github.com/hexgrad/misaki/issues/51));
the `gold` half has no stated source. That concerns pronunciation data, not
program code, and no eSpeak NG code is distributed in any form.
