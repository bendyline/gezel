...
- **`generalist`** — the generalist-mode A/B suite ([generalist-mode.md](generalist-mode.md)): the two hermetic fanout probes (`fanout-tally`, `fanout-stories`), the plain-chat refactor control (`schema-migration`, deliberately shared with core; its setup messages a pre-recruited Developer directly, so the setting must leave it untouched) and four multi-step gated craftbook runs, ordered cheapest-first. Not a scorecard: run it twice, `--generalist off` and `--generalist on`, through `pnpm eval:ab-generalist`; `generalist-smoke` (3 scenarios, ~2h50m) is the dry-run and n=3 subset. The comparison reads `facts.continuity` (steps, sessions per step, compactions, fanout integrity) alongside pass rate. The bin runs craftbook cells under `--repair-policy runtime` so the harness's repair channel is not a third actor in the comparison; `--repair-policy harness` is the standard-matrix behaviour.
