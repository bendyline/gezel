---
id: whats-new/1.26251
title: 1.26251 — 8 September 2026
order: -26251
...
- **llama.cpp and MLX share their local-turn policy** — task classification, repair tool surfaces, retry allowances, history compaction at 70% pressure, and text salvage — with one contract suite run against both real provider loops, so a fix to how one engine handles a turn now reaches the other too.
...
- **The scorecard is one file per sweep**, so results measured on different machines can be added independently without colliding.
...
- An investigation into a 96-token thinking cap that can end a model's reasoning mid-sentence landed, along with an A/B harness for choosing budgets. No budget change ships in this release.
