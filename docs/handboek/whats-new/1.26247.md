...
- **A distribution profile now sits beside the security policy.** It lets a build declare that it may not download or install executable code; engine downloads, the Copilot SDK, Playwright and npm installs are then refused, and an inherited environment variable cannot argue the profile out of its own restrictions. Model weights and catalogs count as data, so such a build can still ship a packed-in starter model that answers its first prompt, speaks and transcribes with nothing downloaded.
