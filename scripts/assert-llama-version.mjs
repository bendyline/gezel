...
export function parseLlamaPin(contents) {
  const tag = contents.match(/^tag=(b\d+)\s*$/m)?.[1];
  const commit = contents.match(/^commit=([0-9a-f]{40})\s*$/m)?.[1];
  if (!tag || !commit) {
    throw new Error('llama VERSION must contain tag=b<number> and a 40-character commit');
  }
  return { buildNumber: Number.parseInt(tag.slice(1), 10), commit, tag };
}
