...
- **pnpm** — build-time fetch [fetch-pnpm.mjs](packages/app/scripts/fetch-pnpm.mjs) + runtime extract [extract-pnpm.ts](packages/app/src/supervisor/extract-pnpm.ts) land the pinned pnpm binary at `~/.gezel/bin/pnpm[.exe]`. Supervisor sets `GEZEL_PNPM_PATH` to the resolved absolute path; `resolvePnpmBinary` in the service reads it, falling back to `pnpm` on PATH when unset. Version pin lives in [pnpm-version.ts](packages/app/src/pnpm-version.ts); bump via `node scripts/bump-pnpm.mjs <version>`. Placeholder (zeros) shas are a hard build error — set `GEZEL_PNPM_SKIP=1` at build time to opt out (dev iteration without bumping).
...
Both binaries are exposed via `process.env` (`GEZEL_PNPM_PATH` / `GEZEL_NODE_PATH`) to spawned children that inherit the env — no explicit per-spawn wiring needed beyond the existing `{ ...process.env }` pattern.
