...
const authoringGildeRoot = findGstackAuthoringGildeRoot();
const authoringRoot = authoringGildeRoot ? gstackAuthoringDir(authoringGildeRoot) : '';
const dataRoot = authoringGildeRoot ? join(authoringGildeRoot, 'data', 'craftbook-templates') : '';
const snapshotRoot = join(authoringRoot, 'snapshots');
const evalRoot = join(authoringRoot, 'evals');
const wave = authoringGildeRoot ? readGstackWaveConfig(authoringGildeRoot) : undefined;
...
function freshConversion(book: (typeof WAVE)[number]): CraftbookDoc {
  if (!wave) throw new Error('gstack authoring inputs are unavailable');
...
describe.skipIf(!wave)('shipped skill conversions — regen fidelity', () => {
