...
/**
 * Step policies became generated output after the first gstack wave shipped.
 * Preserve byte fidelity for an old exact-pinned package while requiring new
 * waves that carry policies to reproduce the current writer's full output.
 */
function freshCommittedConversion(book: (typeof WAVE)[number]): CraftbookDoc {
  const converted = freshConversion(book);
...
function expectedEvalArtifactPath(
  book: (typeof WAVE)[number],
  logicalPath: string,
  spec: CraftbookTestSpec,
): string {
  const workPath = spec.setup.craftbookParams?.workPath;
  if (!workPath) return logicalPath;
  expect(workPath, `${book.id}: task-scoped artifact eval needs a stable workPath`).toBeTypeOf(
    'string',
  );
  return `${workPath}/${logicalPath}`;
}
...
describe('shipped skill conversions — regen fidelity', () => {
  for (const book of WAVE) {
    it(`${book.id}: generated bytes match the Gilde authoring source`, () => {
      ...
      const doc = freshCommittedConversion(book);
  ...
  it('all nine eval sources exercise their named workflow with runtime attribution', () => {
    for (const book of WAVE) {
      ...
      const artifactPath = expectedEvalArtifactPath(book, workflow!.review.artifactPath, spec);
      ...
      expect(spec.rubric.artifact.path).toBe(artifactPath);
