...
const SURFACE_LABEL: Record<ErrorReportSurface, string> = {
  ...
  'main-process': 'main process crashed',
...
export function formatErrorReport(input: ErrorReportInput): string {
  ...
  blocks.push(
    '---\nReported from Gezel. Nothing is sent automatically. ' +
      'I reviewed this text before sending; it contains no logs, file paths, or personal data.',
  );
