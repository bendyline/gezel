...
describe('craftbook exact tool policy', () => {
  it('allows removing artifact search while retaining artifact output', () => {
    expect(CraftbookStepToolPolicySchema.parse({
      outputMedium: 'artifact',
      disallowTools: ['list_artifacts', 'grep_artifact', 'read_doc_as_markdown'],
    }).disallowTools).toEqual(['list_artifacts', 'grep_artifact', 'read_doc_as_markdown']);
  });
  it('rejects removal of workflow safety and required output tools', () => {
    expect(CraftbookStepToolPolicySchema.safeParse({ disallowTools: ['advance_task_step'] }).success).toBe(false);
    expect(CraftbookStepToolPolicySchema.safeParse({ outputMedium: 'artifact', disallowTools: ['write_artifact'] }).success).toBe(false);
  });
  it('accepts a fixed read-action ceiling but requires its artifact writer in the observation step', () => {
    ...
    }).allowTools).toEqual(['read_artifact', 'read_artifacts']);
    expect(CraftbookStepToolPolicySchema.safeParse({
      outputMedium: 'artifact',
      allowTools: ['read_artifact', 'read_artifacts'],
    }).success).toBe(false);
    expect(CraftbookStepToolPolicySchema.parse({
    ...
    }).allowTools).toContain('write_artifact');
  });
