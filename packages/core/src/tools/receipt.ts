/**
 * The record a tool call leaves in the transcript.
 *
 * Both hosts persist the same `ChatMessageToolCall`, and the user reads the
 * same receipt whichever host produced it: a plain-language one-liner for
 * the arguments, the full arguments behind an expander, and a bounded
 * copy of the result. Before this module the two hosts filled those fields
 * two different ways, so the same call read differently on a phone.
 *
 * What a host feeds back to the model is a separate question with a
 * host-specific budget; this module is only about what is persisted.
 */
import type { ChatMessageToolCall } from '../schemas/gezel.js';
...
export interface ToolReceiptInput {
  ...
  /** When the call began, in epoch milliseconds. */
  startedAtMs: number;
  durationMs: number;
  success: boolean;
  errorMessage?: string;
  /** The tool's full result; it is summarised, never stored whole past the cap. */
  resultText?: string;
  /** A larger persisted-result budget for calls whose evidence must survive. */
  resultCap?: number;
}

/** The persisted receipt for one tool call, identical on every host. */
export function buildToolReceipt(input: ToolReceiptInput): ChatMessageToolCall {
  const argsSummary = humanizeToolCall(input.name, input.args) ?? summarizeToolArgs(input.args);
  const argsFull = renderFullToolArgs(input.args);
  const result = summarizeToolResult(input.resultText, input.resultCap);
  return {
    name: input.name,
    at: new Date(input.startedAtMs).toISOString(),
