 ...
 * in core/deliverable.ts. Desktop and portable hosts share these pure
 * tool-name sets; host capability and authorization ceilings apply afterwards.
 *
...
import type { DeliverableKind } from '../deliverable.js';
import { deliverableKindForStep, isProseDocPath, stepDeliverablePath } from '../deliverable.js';
 ...
 */
export function unionStepKit(
  activeStep: Parameters<typeof stepToolKit>[0],
  steps: ReadonlyArray<Parameters<typeof stepToolKit>[0]>,
): StepKit | null {
  ...
  return { kind, path, tools } as StepKit;
}
