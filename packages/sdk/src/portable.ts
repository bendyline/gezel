/**
 * Portable script SDK for runtimes without Node. A host supplies the
 * initialization payload and a permission-checked RPC transport per run;
 * importing this entry does not access stdin or any platform APIs.
 */

import type { IndexReadinessReport, WorkspaceIndexStatus } from './index-readiness.js';
import type { ScriptTransport } from './transport.js';
...
export type { ScriptInit, ScriptTransport } from './transport.js';
...
export interface GezelSDKOptions {
  /** Optional runtime log sink, in addition to the script.log notification. */
  log?: (...args: unknown[]) => void;
}

/**
 * Create a script context with its own input, transport, and output stamp.
 * The transport is privileged host infrastructure: it must enforce the run's
 * capability grants on every call, including notifications. This factory
 * provides the script-facing API; it does not grant filesystem or network access.
 */
export function createGezelSDK(rpc: ScriptTransport, options: GezelSDKOptions = {}): GezelSDK {
  ...
  return {
    ...
    output(value: unknown): void {
      if (outputStamped) {
        throw new Error(
          'gezel.output() called more than once — a script may only stamp one output',
        );
      }
      outputStamped = true;
    ...
    task: {
      ...
      appendNote: (ref, text, stepId) =>
        rpc.call<TaskNote>('task.appendNote', { ref, text, stepId }),
