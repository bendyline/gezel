 ...
 * small script instead of hand-emitting the rows through `writeFile`.
 *
 ...
 * standing steer for the model tiers that reach for `writeFile` by
 ...
 * reports and prose that merely cite data are normal writeFile work.
 */
...
For a json/csv/tsv deliverable BUILT FROM other files (transform, dedup, convert, aggregate): write a small Node script and execute it — \`derive_file({ script, outputPath })\`, or a script file + \`run_nodejs_script\`. Hand-typing rows loses data. Reports and prose that merely cite data are normal \`writeFile\` work.`;
...
export const PromptDeriveByExecution: Behavior = {
  ...
  description:
    'Steers models to produce derived data files (json/csv computed from other files) by executing a script (`derive_file` / `run_nodejs_script`) instead of hand-emitting rows through `writeFile` — hand-typed derived data loses records. Scope to the local fleet models that do data work; prose/report deliverables are unaffected.',
