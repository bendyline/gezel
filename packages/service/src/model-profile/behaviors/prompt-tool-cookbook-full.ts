...
| "create a task" / "add a task to do X" | \`create_task({ project, title, description, steps, assignee: {kind:"gezel", gezelId} })\` — \`steps\` is an inline craftbook (or use \`craftbookId\` to copy from the catalog); \`assignee\` requires a real \`gezelId\`; if the role doesn't exist yet, call \`ensure_gezel\` FIRST and use the returned id |
