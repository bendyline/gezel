...
export type CodexNativeProvider = 'llama-cpp' | 'mlx' | 'ds4';

const CODEX_NATIVE_PROVIDERS = new Set<ProviderName>(['llama-cpp', 'mlx', 'ds4']);
const DEFAULT_PROVIDER_DEADLINE_MS = 8_000;
const PROVIDER_DEADLINE_EXPIRED = Symbol('codex-provider-deadline-expired');

export interface CodexSetupModelSourceOptions {
...
/**
 * Build the model callback consumed by CodexSetupManager.
 *
 ...
 */
export function createCodexSetupModelSource(
  opts: CodexSetupModelSourceOptions,
  ...
  return async (provider) => {
    ...
    const result = await beforeDeadline(async (signal) => {
      const nativeProvider = isCodexNativeProvider(provider) ? provider : null;
  ...
  try {
    ...
    return await Promise.race<T | typeof PROVIDER_DEADLINE_EXPIRED>([
      ...
      new Promise<typeof PROVIDER_DEADLINE_EXPIRED>((resolve) => {
        timer = setTimeout(() => {
          controller.abort(new DOMException('Codex model discovery timed out', 'TimeoutError'));
...
function isCodexNativeProvider(provider: ProviderName): provider is CodexNativeProvider {
  return CODEX_NATIVE_PROVIDERS.has(provider);
}
...
async function resolveCatalogModel(
  catalog: CatalogService,
  modelId: string,
): Promise<{ name: string; supportsTools: boolean } | null> {
  ...
  try {
    ...
    return { name: detail.manifest.name, supportsTools: detail.manifest.supportsTools };
  } catch {
    return null;
  }
}
