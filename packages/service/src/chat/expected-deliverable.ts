...
export function formatExpectedDeliverableAnnotation(
  ...
  if (path && isExpectedBinaryDocumentDeliverablePath(path)) {
    return `\n\n[Deliverable expected as a REAL BINARY DOCUMENT OR MEDIA FILE at \`${path}\`. Preserve that exact format — a markdown outline, HTML page, or similarly named text file is not the deliverable. Use the installed DocBlocks production craftbook: author the source as Markdown, then follow its conversion, visual-preview, and artifact-save steps for the requested target. Do NOT hand-build HTML/OOXML or call \`write_file\` with prose or base64 for this binary file. If that production workflow is not on your roster, reply that the exact-format deliverable is blocked instead of silently substituting another format.]`;
  }
