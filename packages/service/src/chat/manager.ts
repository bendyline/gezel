...
import { installedModelInfos } from '../providers/native/inference.js';
...
export class ChatManager extends LocalEngineRuntime {
  protected override async prefillCacheSession(sessionId: string): Promise<boolean> {
    const state = await this.ensureState(sessionId);
    if (!state.session?.prefillOnly) return false;
    await state.session.prefillOnly();
    return true;
  }
  ...
  protected declare readonly store: Store;
  ...
  private async retireMachineEngineSessions(): Promise<void> {
    ...
    for (const [sessionId, state] of this.states) {
      if (!isNativeLocalProvider(state.record.providerName)) continue;
  ...
  private async retireSingletonMachineEngineProviders(): Promise<void> {
    for (const [name, provider] of this.providers) {
      if (!isNativeLocalProvider(name)) continue;
  ...
  async messageGezel(args: MessageGezelArgs): Promise<{
    ...
    const fileHandoffKey = requestedFilePath
      ? [
          ...
          JSON.stringify(args.fileTurnIntent ?? null),
        ].join('\u0000')
      : null;
    ...
    const dispatchTargetSend = () => {
      ...
      const targetSend = this.sendWithBusyRetry(session.id, seed, {
        ...(args.fileTurnIntent ? { fileTurnIntent: args.fileTurnIntent } : {}),
  ...
  async sendWithMentions(args: {
    ...
    for (const mention of targets) {
      ...
      this.trackBackground(
        this.sendWithBusyRetry(mentionSession.id, text, {
          fileTurnIntent: args.fileTurnIntent,
        }).catch((err) => {
          log.error(
    ...
    try {
      ...
      while (true) {
        ...
        const sendOpts: SendAndWaitOpts = {
          ...
          ...(continuations === 0 && opts?.fileTurnIntent
            ? { fileTurnIntent: opts.fileTurnIntent }
            : {}),
  ...
  async listModelsForProvider(name: ProviderName, signal?: AbortSignal): Promise<ModelInfo[]> {
    ...
    if (localManager) {
      try {
        ...
        return installedModelInfos(installed);
      } catch {
  ...
  async getProviderForModel(
    ...
    return this.getLocalProviderForModel(name, modelId, opts, () => this.ensureProvider(name));
  }
