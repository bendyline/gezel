...
- A 3x3 grid of cells.<|"|>}`;
  ...
  describe('embedded, unterminated envelope (no closing <tool_call|>)', () => {
    ...
    // `…Owner |<channel|><|tool_call>call:writeFile{content:<|"|># …` with no
    ...
    it('salvages a writeFile whose opener is mid-prose and never closes', () => {
      const text =
        'The Work plan section must be a Markdown table with columns exactly ' +
        '`ID | Task | Owner |<channel|><|tool_call>call:writeFile{content:<|"|>' +
    ...
    it('captures a path key when the model does emit one before content', () => {
      const text =
        'prose prose |<channel|><|tool_call>call:writeFile{path:<|"|>plan.md<|"|>,content:<|"|># Plan\nline2';
      const spans = findGemmaNativeToolCallSpans(text, TOOLS);
      expect(spans).toHaveLength(1);
    ...
    it('does NOT fabricate a call from prose that merely mentions a tool name', () => {
      const text =
        'The developer should call writeFile to save the plan. Owner | writeFile matters.';
    ...
    it('the span spans from the opener to end-of-text so the marker is stripped', () => {
      const text = 'keep me <|tool_call>call:writeFile{content:<|"|>hi';
      const spans = findGemmaNativeToolCallSpans(text, TOOLS);
      expect(spans).toHaveLength(1);
  ...
  describe('peg-parse dropped shapes (cbmx-20260720-195716)', () => {
    ...
    it('parses back-to-back terminated readFile envelopes (craftbook-crm-update-batch)', () => {
      const text =
        '<|tool_call>call:readFile{path:<|"|>source/schema.md<|"|>}<tool_call|>' +
        '<|tool_call>call:readFile{path:<|"|>source/call-notes.md<|"|>}<tool_call|>';
      const spans = findGemmaNativeToolCallSpans(text, TOOLS);
    ...
    it('parses a writeFile embedded after channel-markup prose (craftbook-crm-update-batch)', () => {
      const text =
        'The CSV must have a specific 1<channel|><|tool_call>call:writeFile{content:<|"|>' +
    ...
    it('parses a terminated-brace call whose <tool_call|> closer is followed by prose (craftbook-juice-pass)', () => {
      const text =
        '<|tool_call>call:writeFile{content:<|"|><!DOCTYPE html>\n<html lang="en">\n</html>' +
    ...
    describe('headless body (engine ate the `<|tool_call>call:writeFile{` prefix)', () => {
      ...
      it('infers writeFile from a leading content+path body and strips only the span', () => {
        const text =
