...
/**
 * Locate the run of one to three plain double quotes — or backticks — that
 * closes an unterminated native string. Candidates are closer runs followed
 * by another argument (`, key:`), tried from the last one backwards, and
 * accepted only when everything after them parses as the rest of the
 * argument object through its closing brace — so a `", key:` sequence inside
 * the content itself cannot win. With no such candidate, a closer run right
 * before the closing brace ends the string. Returns the span or null.
 *
 * Backticks are a closer because gemma4-12b ends a multi-line HTML `content`
 * with a JavaScript template-literal backtick as readily as with Python's
 * `"""`: eight identical "Missing required fields: path" rejections on one
 * invoice child while the fanout barrier held the host (2026-09-20).
 */
function lastMisclosedStringBoundary(rest: string): { start: number; end: number } | null {
  const argBoundary = /(?:"{1,3}|`{1,3})(?=\s*,\s*[a-zA-Z_][a-zA-Z0-9_-]*\s*:)/g;
  ...
  const closing = rest.match(/(?:"{1,3}|`{1,3})(?=\s*\}\s*(?:<tool_call\|>)?\s*$)/);
...
class GemmaNativeBodyParser {
  ...
  private readGemmaString(): string {
    ...
    // A value the model closed with plain quotes or backticks and then
    // continued with `,key:<|"|>` makes the NEXT value's opener look like
    // this string's closer: `content:<|"|>...</html>""",path:<|"|>x.html<|"|>}`
    // swallowed `""",path:` into content and the call lost its `path`
    // fifteen times in a row (gemma4-12b-q4 invoice child, 2026-09-21).
    // Cut at the closer run and resume at the comma so the real argument
    // parses normally.
    const misclosed = value.match(/(?:"{1,3}|`{1,3})(?=\s*,\s*[a-zA-Z_][a-zA-Z0-9_-]*\s*:\s*$)/);
    if (misclosed?.index !== undefined) {
      this.pos += misclosed.index + misclosed[0].length;
      return value.slice(0, misclosed.index);
    }
...
export function parseJsonEnvelopeToolCall(
  ...
  if (exact && knownToolNames.has(exact.name))
    return { ...exact, matchStart: text.indexOf('{'), matchEnd: text.lastIndexOf('}') + 1 };
...
export { extractReasoning, stripReasoningTags } from '@bendyline/gezel';
