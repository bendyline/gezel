...
export interface TaskRetryDeps {
  ...
  tasks: Pick<TaskManager, 'get' | 'setStatus' | 'appendNote' | 'resetStepRecoveryBudget'>;
...
export async function retryPausedTask(
  ...
  const step = stepId ? task.craftbook.steps.find((s) => s.id === stepId) : undefined;
  if (stepId) {
  ...
  if (!stepId || !step) {
    return { task: active, dispatched: false, reason: 'no-active-step' };
  }
  const gezelId = stepOwnerGezelId(active, step);
  if (!gezelId) {
    return { task: active, dispatched: false, reason: 'no-assignee' };
  }
  ...
  if (project && !projectAllowsAmbientWork(project)) {
    log.info(`[tasks] retry dispatch skipped for ${active.ref}: project not active`);
    return {
      task: active,
      dispatched: false,
      gezelId,
  ...
  deps.taskRunner.enqueueHandoff({
    ...
    resumeExisting: true,
    ...(active.nightShift?.enabled === true ? { nightShift: true } : {}),
    ...(step.lastActivatedAt ? { activationAt: step.lastActivatedAt } : {}),
  });
  log.info(`[tasks] retry dispatched: ${active.ref} step ${stepId} → ${assigneeName ?? gezelId}`);
  return { task: active, dispatched: true, gezelId, ...(assigneeName ? { assigneeName } : {}) };
}
