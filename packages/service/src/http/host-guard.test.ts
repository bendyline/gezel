...
describe('hostGuard', () => {
  it('allows loopback hosts and rejects LAN IPs while remote serving is stopped', async () => {
    ...
    expect((await app.request('/', { headers: { host: '127.0.0.1:43935' } })).status).toBe(200);
    expect((await app.request('/', { headers: { host: '192.168.1.20:43936' } })).status).toBe(403);
  });

  it('allows literal LAN IP hosts only while the remote listener is active', async () => {
    let listening = true;
    const app = guardedApp(() => listening);
    expect((await app.request('/', { headers: { host: '192.168.1.20:43936' } })).status).toBe(200);
    listening = false;
    expect((await app.request('/', { headers: { host: '192.168.1.20:43936' } })).status).toBe(403);
  });
  ...
  it('continues to reject domain names when remote serving is active', async () => {
    ...
    expect((await app.request('/', { headers: { host: 'attacker.example:43936' } })).status).toBe(
      403,
    );
  });
