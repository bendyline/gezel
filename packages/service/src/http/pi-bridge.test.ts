...
describe('buildPiBridgeApp', () => {
  ...
  it('applies the Connected Apps master switch before inference', async () => {
    const app = buildPiBridgeApp(context({ enabled: false }));
  ...
  it('adds security/ownership headers, rejects rebound hosts, and does not enable CORS', async () => {
    ...
    expect(forbidden.headers.get(PI_BRIDGE_MARKER_HEADER)).toBe('1');
...
describe('createPiBridgeController', () => {
  it('binds loopback, reports its origin, and stops idempotently', async () => {
    const app = buildPiBridgeApp(context());
    const controller = createPiBridgeController({
    ...
    expect(response.headers.get(PI_BRIDGE_MARKER_HEADER)).toBe('1');
  ...
  it('rejects an out-of-range port', () => {
    expect(() =>
      createPiBridgeController({ fetch: () => () => new Response(), port: 70_000 }),
    ).toThrow(/Invalid pi bridge port/);
  });
});
