...
/** Hard cap on any single chat attachment. */
...
export function imagesRoutes(ctx: ServiceContext): Hono {
  ...
  app.post('/:projectId/attachments', async (c) => {
    ...
    if (body.byteLength > MAX_ATTACHMENT_BYTES) {
      return c.json(
        {
          error: `attachment too large: ${body.byteLength} bytes (max ${MAX_ATTACHMENT_BYTES})`,
        },
        413,
      );
    }
  ...
  app.post('/:projectId/sessions/:sessionId/images', async (c) => {
    ...
    if (body.byteLength > MAX_ATTACHMENT_BYTES) {
      return c.json(
        { error: `image too large: ${body.byteLength} bytes (max ${MAX_ATTACHMENT_BYTES})` },
        413,
      );
    }
