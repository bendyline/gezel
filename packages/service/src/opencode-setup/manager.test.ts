...
describe('OpenCodeSetupManager', () => {
  ...
  it('publishes a config OpenCode can parse after {file:} substitution', async () => {
    ...
    expect(provider?.models[`gezel:${maya.id}`]).toMatchObject({
      tool_call: true,
    ...
    expect(parsed.model).toBe(`gezel/gezel:${maya.id}`);
  ...
  it('reaches configured, mints a scoped credential, and starts the bridge', async () => {
    ...
    const after = await f.manager.configure({ model: `gezel:${maya.id}` });

    expect(after.state).toBe('configured');
    expect(after.configuredModel).toBe(`gezel:${maya.id}`);
  ...
  it('reports update-needed when the credential is revoked, and repairs on reconfigure', async () => {
    ...
    const repaired = await f.manager.configure({ model: `gezel:${maya.id}` });
  ...
  it('reports OpenCode as missing without blocking setup', async () => {
    ...
    await expect(f.manager.configure({ model: `gezel:${maya.id}` })).resolves.toMatchObject({
      state: 'configured',
    });
  });
