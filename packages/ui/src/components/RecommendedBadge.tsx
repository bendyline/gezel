 ...
 * `isRecommendedModel` gate — curated `recoScore`, fully-open license, and no
 * explicit `supportsTools: false`. That is the exact predicate the first-run
 * picker uses (see `packages/core/src/recommendation.ts`), so the badge and
 * the auto-pick can never disagree.
 *
 ...
 * audio — and simply renders nothing for un-scored, restricted, or tool-less
 * models.
 */
export function RecommendedBadge({
  manifest,
}: {
  manifest: { recoScore?: number; licenseClass?: LicenseClass; supportsTools?: boolean };
}) {
