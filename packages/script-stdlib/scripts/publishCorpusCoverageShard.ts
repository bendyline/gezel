...
export const meta = defineScript({
  ...
  description:
    'Action: copy the exact paths and records of one verified review batch into its coverage shard after the read-evidence gate passes.',
  kind: 'action',
  inputs: {
    batchesFile: {
      type: 'string',
      description: 'Artifact path of the published batch array.',
      required: true,
    },
...
try {
  ...
  throw new Error(`${batchesFile} is not valid JSON.`);
}
...
const batch = batches.find(
  (value) =>
    ...
    typeof value === 'object' &&
    !Array.isArray(value) &&
    (value as Record<string, unknown>).batchNumber === number,
...
if (
  ...
  batch.paths.length !== batch.records.length ||
  batch.paths.some((value) => typeof value !== 'string') ||
  batch.records.some((value) => typeof value !== 'string')
) {
