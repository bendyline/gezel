...
export const meta = defineScript({
  ...
  description: 'Action: summarize a published PR corpus manifest and batch array into small scope metadata without sending every path and record to the model.',
  kind: 'action',
  inputs: {
    manifestFile: { type: 'string', description: 'Exact artifact path of the PR file manifest.', required: true },
    batchesFile: { type: 'string', description: 'Artifact path of the runtime-published batch array.', required: true },
    outFile: { type: 'string', description: 'Artifact path of the compact scope summary.', required: true },
  },
  outputs: {
...
try {
  ...
  throw new Error(`Could not parse PR scope inputs: ${error instanceof Error ? error.message : String(error)}`);
}
if (!manifest || typeof manifest !== 'object' || Array.isArray(manifest) || !Array.isArray(batches) || batches.length === 0) {
...
const ranges = batches.map((value, index) => {
  ...
  if (batch.batchNumber !== index + 1 || !Number.isSafeInteger(batch.start) || !Number.isSafeInteger(batch.end) || !Array.isArray(batch.paths)) {
