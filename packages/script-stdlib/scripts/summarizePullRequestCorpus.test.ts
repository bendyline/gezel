...
const h = vi.hoisted(() => {
  ...
  return {
    ...
    gezel: {
      input: {
        manifestFile: 'data/pr/attachments/pr-files.json',
        batchesFile: 'tasks/1/pr-review/batches.json',
        outFile: 'tasks/1/pr-review/scope-data.json',
      },
...
beforeEach(() => {
  ...
  h.reset();
  h.files.set(
    'data/pr/attachments/pr-files.json',
    JSON.stringify({
      pullRequest: 60,
      totalFiles: 3,
      files: [{ path: 'src/a.ts' }, { path: 'src/b.ts' }, { path: 'tests/a.test.ts' }],
    }),
  );
  h.files.set(
    ...
    JSON.stringify([
      {
        ...
        records: ['data/a.md', 'data/b.md'],
      },
      { batchNumber: 2, start: 3, end: 3, paths: ['tests/a.test.ts'], records: ['data/c.md'] },
    ]),
  );
});
describe('summarizePullRequestCorpus', () => {
  it('publishes counts and ranges without copying the huge path/record arrays', async () => {
    vi.resetModules();
    await import('./summarizePullRequestCorpus');
    ...
    expect(summary.ranges).toEqual([
      { number: 1, start: 1, end: 2, files: 2 },
      { number: 2, start: 3, end: 3, files: 1 },
    ]);
  ...
  it('fails closed when the manifest file count disagrees with its array', async () => {
    h.files.set(
      'data/pr/attachments/pr-files.json',
      JSON.stringify({ totalFiles: 4, files: [{ path: 'src/a.ts' }] }),
    );
    vi.resetModules();
